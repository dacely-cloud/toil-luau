...
"production" !== process.env.NODE_ENV && function () {
  ...
  __exports.jsx = function (__type, config, maybeKey) {
    let trackActualOwner;
    trackActualOwner = 1e4 > ReactSharedInternals.recentlyCreatedOwnerStacks++;
    return jsxDEVImpl(__type, config, maybeKey, !1, trackActualOwner ? Error("react-stack-top-frame") : unknownOwnerDebugStack, trackActualOwner ? createTask(getTaskName(__type)) : unknownOwnerDebugTask);
  };
  __exports.jsxs = function (__type, config, maybeKey) {
    let trackActualOwner;
    trackActualOwner = 1e4 > ReactSharedInternals.recentlyCreatedOwnerStacks++;
    return jsxDEVImpl(__type, config, maybeKey, !0, trackActualOwner ? Error("react-stack-top-frame") : unknownOwnerDebugStack, trackActualOwner ? createTask(getTaskName(__type)) : unknownOwnerDebugTask);
  };
}();
